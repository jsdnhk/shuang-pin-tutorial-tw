...
LEGAL_COMPOSITE = {
  ...
  :j => %w(i ia ian iang iao ie in ing iu ü üe üan ün iong),
  :q => %w(i ia ian iang iao ie in ing iu ü üe üan ün iong),
  :x => %w(i ia ian iang iao ie in ing iu ü üe üan ün iong),
...
SHENG_YUN_WORD_TABLE = {
  ...
  :j => {:i=>"基", :ia=>"家", :ie=>"街", :iao=>"交", :iu=>"究", :ian=>"堅", :in=>"今", :iang=>"江", :ing=>"京", :ü=>"居", :üe=>"決", :üan=>"捐", :ün=>"軍", :iong=>"窘"},
  :q => {:i=>"欺", :ia=>"搯", :ie=>"切", :iao=>"敲", :iu=>"秋", :ian=>"千", :in=>"親", :iang=>"腔", :ing=>"清", :ü=>"區", :üe=>"缺", :üan=>"圈", :ün=>"群", :iong=>"窮"},
  :x => {:i=>"希", :ia=>"瞎", :ie=>"些", :iao=>"消", :iu=>"休", :ian=>"先", :in=>"新", :iang=>"香", :ing=>"星", :ü=>"虛", :üe=>"靴", :üan=>"宣", :ün=>"詢", :iong=>"兄"},
...
def rand_words(num = COUNT, display_delim: "|")   # return [words(str), display lines(ary of str)] array
  result = []
  display = []
  display_py = []
  display_zy = []
  ...
  num.times do
    ...
    sheng_zy = SHENG_MU[sheng].to_s.strip
    yun = LEGAL_COMPOSITE[sheng].sample.to_sym
    yun_zy = YUN_MU[yun.to_sym].to_s.strip
    shengyun = (sheng.to_s.strip + yun.to_s.strip).sub(/^o/,'')
    zy = case shengyun  # special cases needed to fine-adjust
           #:y => %w(a an ang ao e i in ing ong ou u üan üe ün),
           when 'yong'
             YUN_MU[:iong]
           when 'yu', 'yü'
             YUN_MU[:ü]
    ...
    cw = SHENG_YUN_WORD_TABLE.fetch(sheng).fetch(yun) rescue print("have no word when #{sheng}#{yun}"); ''
